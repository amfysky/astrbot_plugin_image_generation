...
- 纯文本模型也能图生图：消息中的图片会以 `message:1`、`reply:1`、`avatar:QQ号` 这类文本句柄注入 LLM 提示，图片本身不进入模型上下文，非多模态聊天模型也能指定参考图。
- 模型动态切换：通过指令查看和切换模型。
...
- 消息互动：生图结果可引用触发消息回复（指令直接发送与 AI 唤醒交付均支持）；生图进行中可给触发消息贴进度表情，完成后自动移除（表情反应仅 aiocqhttp/NapCat 等 QQ 协议端支持）。
...
| 配置名           | 默认值 | 说明                                                                                                              |
| :--------------- | :----- | :-------------------------------------------------------------------------------------------------------------- |
| 引用触发消息回复 | 开启   | 开启后生图结果会引用触发生图的原始消息回复，涵盖 `/生图` 指令直接发送与 AI 唤醒交付（`send_message_to_user`）两条路径；AI 可通过 `quote` 参数选择是否引用。 |
| 生图进度表情反应 | 开启   | 开启后生图进行中给触发消息贴进度表情表示“生成中”，任务结束后自动移除。仅 aiocqhttp（QQ，如 NapCat/Lagrange）协议端支持。 |
| 进度表情 ID 列表 | `[66]` | 生图进行中贴上的 QQ 表情 ID，可配置多个。表情 ID 可参考 OneBot/NapCat 表情对照表；如需停用请关闭“生图进度表情反应”。      |

### LLM 工具参考图配置

控制 LLM 生图工具如何拿到当前消息里的图片。非多模态（纯文本）聊天模型看不到消息图片，只能通过句柄机制进行图生图。

| 配置名               | 默认值 | 说明                                                                                                                                             |
| :------------------- | :----- | :----------------------------------------------------------------------------------------------------------------------------------------------- |
| 注入图片句柄提示     | 开启   | 消息中存在图片、引用图片或 @ 用户时，在 LLM 请求的系统提示中追加一段纯文本句柄清单（`message:1`、`reply:1`、`avatar:QQ号`）。图片本身不进入模型上下文。 |
| 未指定参考图时自动引用 | 开启   | LLM 调用生图工具且未填写任何参考图、头像和人设参数时，自动使用当前消息图片、引用消息图片和 @ 用户头像作为参考图；关闭后必须由模型显式填写句柄。       |
...
LLM 生图工具支持 `preset`、`persona`、`aspect_ratio`、`resolution`、`image_count`、`avatar_references` 和 `reference_images` 参数。`avatar_references` 可填写 `self`、`sender` 或用户 ID；`reference_images` 支持图片句柄、网络图片 URL，本地路径仅允许当前会话 workspace、AstrBot temp 目录和本插件数据目录。

#### 图片句柄

聊天模型不支持图片输入时，它既看不到用户发的图，也拿不到图片 URL，无法自行发起图生图。插件为此把消息中的图片索引成短文本句柄注入提示词，模型只需把句柄原样填回工具参数，插件在后端解析成图片字节直接送给生图模型——图片字节全程不进入聊天模型的上下文。

| 句柄            | 含义                             | 可填入                              |
| :-------------- | :------------------------------- | :---------------------------------- |
| `message:N`     | 用户本条消息中的第 N 张图片      | `reference_images`                  |
| `reply:N`       | 被引用消息中的第 N 张图片        | `reference_images`                  |
| `avatar:<QQ号>` | 指定用户的头像                   | `reference_images`、`avatar_references` |
| `avatar:sender` | 发送者头像                       | `reference_images`、`avatar_references` |
| `avatar:self`   | 机器人自己的头像                 | `reference_images`、`avatar_references` |

`message`/`reply` 也可不带序号或写 `:all`，表示该来源的全部图片。句柄清单只在消息确实包含图片、引用图片或 @ 用户，且当前模型支持图生图时才注入。
